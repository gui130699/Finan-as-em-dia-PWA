...
def listar_parcelados_pendentes(user_id):
    ...
    try:
        ...
        response = supabase.table('lancamentos')\
            .select('numero_contrato, descricao, total_parcelas, valor')\
            .eq('usuario_id', user_id)\
        ...
        for contrato in contratos.keys():
            response_status = supabase.table('lancamentos')\
                .select('status')\
                .eq('numero_contrato', contrato)\
                .execute()
            
            for lanc in response_status.data:
                if lanc['status'] == 'pago':
        ...
        return [c for c in contratos.values() if c['parcelas_pendentes'] > 0]
        
    except Exception as e:
        print(f"Erro ao listar parcelados: {e}")
        return []

def quitar_parcelado_integral(numero_contrato):
    """Quita todas as parcelas pendentes de um contrato"""
    try:
        supabase = database.conectar()
